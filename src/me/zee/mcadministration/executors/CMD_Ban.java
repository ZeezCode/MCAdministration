...
public class CMD_Ban implements CommandExecutor {
	 ...
	 */
	public CMD_Ban(MCAdministration plugin) {
		this.plugin = plugin;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length >= 2) {
			...
			if (plugin.dbHandler.logAction("Ban", target_ply.getUniqueId(), (calling_ply == null ? null : calling_ply.getUniqueId()), reason, 0l, plugin.util.getTimestamp())) {
				plugin.util.banPlayer(calling_ply, target_ply, reason, true);
				plugin.util.sendMessage(sender, "You have permanently banned " + target_ply.getName() + " for: " + reason);
			} else {
				plugin.util.sendError(sender, "Failed to log action! Player was not banned!");
			}
		} else {
			plugin.util.sendError(sender, "Invalid command format! Use /ban <player> <reason>");
		}
