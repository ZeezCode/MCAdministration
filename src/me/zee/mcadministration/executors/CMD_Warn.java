...
public class CMD_Warn implements CommandExecutor {
	 ...
	 */
	public CMD_Warn(MCAdministration plugin) {
		this.plugin = plugin;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (sender instanceof Player) {
			...
			if (!plugin.dbHandler.rankHasPermission(plugin.permission.getPrimaryGroup(calling_ply), "can_warn")) {
		...
		if (args.length >= 2) {
			...
			if (plugin.dbHandler.logAction("Warn", target_ply.getUniqueId(), (calling_ply == null ? null : calling_ply.getUniqueId()), reason, 0l, plugin.util.getTimestamp())) {
				plugin.util.warnPlayer(calling_ply, target_ply, reason);
				plugin.util.sendMessage(sender, "You have warned " + target_ply.getName() + " for: " + reason);
			} else {
				plugin.util.sendError(sender, "Failed to log action! Player was not warned!");
			}
		} else {
			plugin.util.sendError(sender, "Invalid command format! Use /warn <player> <reason>");
		}
