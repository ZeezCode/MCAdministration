...
public class CMD_Slay implements CommandExecutor {
	 ...
	 */
	public CMD_Slay(MCAdministration plugin) {
		this.plugin = plugin;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length >= 2) {
			...
			if (plugin.dbHandler.logAction("Slay", target_ply.getUniqueId(), (calling_ply == null ? null : calling_ply.getUniqueId()), reason, 0l, plugin.util.getTimestamp())) {
				plugin.util.slayPlayer(calling_ply, target_ply, reason);
				plugin.util.sendMessage(sender, "You have slain " + target_ply.getName() + " for: " + reason);
			} else {
				plugin.util.sendError(sender, "Failed to log action! Player was not slain!");
			}
		} else {
			plugin.util.sendError(sender, "Invalid command format! Use /slay <player> <reason>");
		}
